...
function ApiInstance([string]$endpoint, [string]$usename, [string]$password) {
    ...
    [string]$path = "{0}\ps5\Aims.Sdk.dll" -f $installpath
    Add-Type -Path $path
    [string]$path = "{0}\ps5\Newtonsoft.Json.dll" -f $installpath
    Add-Type -Path $path
...
function CheckSchema([string]$config) {
    ...
    <# start new job to isolate schema validation assemblies #>
    Start-Job -ArgumentList $jobParams  -ScriptBlock {
        [string]$installpath = $args.installpath
        [string]$config = $args.config
        [string]$path = "{0}\config.schema.json" -f $installpath
        [string]$schemajson = Get-Content -Path $path

        [string]$NewtonsoftJsonPath = "{0}\ps5\schema\Newtonsoft.Json.dll" -f $installpath
        [string]$NewtonsoftJsonSchemaPath = "{0}\ps5\schema\Newtonsoft.Json.Schema.dll" -f $installpath
        ...
        {
            public static System.Collections.Generic.IList<string> Validate(Newtonsoft.Json.Linq.JObject token, Newtonsoft.Json.Schema.JSchema schema)
            {
...
'@
        ...
        $Token = [Newtonsoft.Json.Linq.JToken]::Parse($config)
        ...
        if ($ErrorMessages.Count -ne 0) {
            throw $ErrorMessages | ForEach-Object {$t = ""} {  $t = "{0}`n{1}" -f $t, $_  } { $t }
        }
